...
- **2026-05-14 — Docs for "Update from upstream first" feature (Todo 9, Niobe docs-writer):** Added `## [Unreleased]` section to CHANGELOG.md with two bullets: (1) "Update from upstream first" checkbox in Create New Worktree and New Copilot Session dialogs (Existing/New branch modes, hidden in PR/Issue), with git fetch+fast-forward and fallback-to-origin semantics; (2) soft-fail prompt on update failure (network, auth, divergence). Added new "#### Update from Upstream First" subsection to README.md under Git Workspace Creation section (after "From Issue #"), explaining the feature targets Existing and New branch modes, default enabled, configurable in Settings under Git/GitHub, with fallback semantics and failure handling. No version bump per release-process directive (Neo owns that as part of release). Key files touched: CHANGELOG.md lines 1-14 (Unreleased section), README.md lines 267-269 (subsection). Feature confirmed to ship with `UpdateSourceBranchBeforeCreate: true` default in LauncherSettings.cs line 73.
