...
**2026-05-10: Warp ActivateNextTab keystroke correction:** The WarpPaneFocuser probe-and-match cycle was sending the wrong keystroke. Initial implementation sent Ctrl+Tab; Warp bindings.rs (lines 323–340) defines ActivateNextTab on Windows as **Ctrl+PageDown**, not Ctrl+Tab. Tank discovered this via E2E testing when live tab switch failed. Renamed IKeyboardSender.SendCtrlTab → SendNextTab; Win32KeyboardSender now sends Ctrl+VK_NEXT (PageDown key). Authoritative source: https://github.com/warpdotdev/warp/blob/main/app/src/util/bindings.rs lines 323–340 (ActivateNextTab on Windows mapped to `ctrl-pagedown`). See session log `2026-05-10T17-03-18-warp-ctrl-pagedown-and-e2e.md` for full context on this and 4 other bugs that were preventing E2E focus from working.
