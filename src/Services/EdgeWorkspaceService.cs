...
internal partial class EdgeWorkspaceService
{
    ...
    /// </summary>
    internal static void WriteSessionSignals(Dictionary<string, long> lastSavedBySession, int signalIntervalMs = 3000)
    {
        try
        {
            var exeDir = AppContext.BaseDirectory;
            var path = Path.Combine(exeDir, "session-signals.js");
            var json = System.Text.Json.JsonSerializer.Serialize(lastSavedBySession);
