...
- **2026-05-10 — RequiresInteractiveDesktop integration trait:** Tests that spawn real terminal windows or depend on global WinEvent hooks (`EVENT_OBJECT_NAMECHANGE`, create/destroy/foreground) are marked `[Trait("Category", "RequiresInteractiveDesktop")]`. Rationale: GitHub-hosted `windows-latest` runners cannot reliably deliver WinEvent hook notifications for `cmd.exe` windows launched with `UseShellExecute = true`, even though those tests remain valuable locally. CI must filter both skip categories with: `dotnet run --project tests/CopilotBooster.IntegrationTests.csproj -c Release -- -notrait "Category=LocalOnly" -notrait "Category=RequiresInteractiveDesktop"`. Prefer class-level traits when every test in the class has the same interactive desktop dependency; use method-level traits only for mixed classes.
